...
import (
	...
	sap_api_output_formatter "sap-api-integrations-purchase-order-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callPurchaseOrderSrvAPIRequirementHeader("A_PurchaseOrder", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callPurchaseOrderSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	url := fmt.Sprintf("A_PurchaseOrderItem('%s')/to_Item", item.PurchaseOrder)
	outputDataItem, err := c.callPurchaseOrderSrvAPIRequirementItem(url, item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callPurchaseOrderSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
